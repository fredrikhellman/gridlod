...
def mapComputations(ecComputeList, printLevel=0):
    ...
    if not client:
        ...
        for TInd, iElement in ecComputeList:
            ...
            ecT = ecworkerMatrixValued.computeElementCorrector(iElement)
            ecTList.append(ecT)
            resultCounter += 1
        if printLevel >= 2:
            print 'Done'
        ...
        computeElementCorrector = lambda ecCompute: ecworkerMatrixValued.computeElementCorrector(ecCompute[1])
...
def setupWorker(world, coefficient, IPatchGenerator, k, clearFineQuantities, printLevel):
    if not client:
        ecworkerMatrixValued.setupWorker(world, coefficient, IPatchGenerator, k, clearFineQuantities)
    else:
        global sendAr
        setupWorkerWrapper = lambda x: ecworkerMatrixValued.setupWorker(*x)
        if hasattr(coefficient, 'rCoarse'):
            ar = client[:].apply_async(setupWorkerWrapper, (world, None, None, k, clearFineQuantities))
            ar.wait()
            ar = client[:].apply_async(ecworkerMatrixValued.hasaBase)
            hasaBase = ar.get()
            if any(h is False for h in hasaBase):
                if printLevel >= 2:
                    print 'Sending large coefficient'
                sendAr = client[:].apply_async(ecworkerMatrixValued.sendar, coefficient._aBase, coefficient._rCoarse)
            else:
                if printLevel >= 2:
                    print 'Sending small coefficient'
                sendAr = client[:].apply_async(ecworkerMatrixValued.sendar, None, coefficient._rCoarse)
        elif hasattr(coefficient, 'aLagging'):
            ar = client[:].apply_async(setupWorkerWrapper, (world, None, None, k, clearFineQuantities))
            ar.wait()
            if printLevel >= 2:
                print 'Sending both coefficients'
            sendAr = client[:].apply_async(ecworkerMatrixValued.sendas, coefficient._aFine, coefficient._aLagging)
        else:
...
def clearWorkers():
    if not client:
        ...
        ar = client[:].apply_async(ecworkerMatrixValued.clearWorker)
        ar.wait()
