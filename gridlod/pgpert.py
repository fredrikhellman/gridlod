...
def updateElementCorrector(ecT, perturbationPatch):
    if ecT is not None:
        euT = ecT.computeErrorIndicatorCoarse(perturbationPatch)
    else:
        euT = np.inf

def computeInitialElementCorrectors(world, k, coefficient, IPatchGenerator):
